...
test.describe("FT-06: Создание нового задания (выбор ответа)", () => {
  ...
  test("Создание задания типа 'Тест без таймера'", async () => {
    ...
    const editorUrl = `${process.env.BASE_URL}/lesson-editor/book/${bookId}/lesson/${lessonId}/section/${sectionId}`;
    ...
    await test.step(`Переход в редактор урока: ${editorUrl}`, async () => {
      ...
      await page.waitForSelector(".lesson-viewer-layout", { timeout: 30000 });
    });
    ...
    await test.step('Нажатие кнопки "Добавить упражнение"', async () => {
      ...
      await addExerciseBtn.waitFor({ state: "visible", timeout: 60000 });
    ...
    await test.step('Выбор "Новый конструктор"', async () => {
      ...
      await newConstructorBtn.waitFor({ state: "visible", timeout: 60000 });
